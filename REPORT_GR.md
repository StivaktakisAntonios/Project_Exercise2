...
**Συγγραφείς:** Θεοδωρίδης Ανέστης, Στιβακτάκης Αντώνιος-Ραφαήλ
...
**Συμπέρασμα:** Η τιμή **m=100** παρέχει βέλτιστη ισορροπία μεταξύ ποιότητας διαμέρισης και ακρίβειας MLP.
...
**Συμπέρασμα:** Η τιμή **T=5** επιτυγχάνει >98% recall με πρακτική απόδοση ερωτημάτων.
...
**Συμπέρασμα:** Η τιμή **nodes=128** παρέχει επαρκή χωρητικότητα με λογικό χρόνο εκπαίδευσης.
...
**Συμπέρασμα:** Τα **3 επίπεδα** παρέχουν την καλύτερη γενίκευση χωρίς υπερπροσαρμογή.
...
**Συμπέρασμα:** Η **λειτουργία ECO** αποδεικνύεται απαραίτητη για ποιότητα σε μεγάλης κλίμακας σύνολα δεδομένων.
...
**Συμπέρασμα:** Η τιμή **k=10** αποτελεί τυπική επιλογή με αποδεδειγμένη αποτελεσματικότητα στη βιβλιογραφία.
...
| SIFT    | 1M     | 128      | ~40 λεπτά         | 72.35%  | 95.00%   | 36.24 |
...
### 5.2 Σύγκριση Λειτουργιών KaHIP

| Σύνολο | Λειτουργία | Χρόνος Διαμέρισης | Recall@1 | AF     | QPS   |
|--------|-----------|-------------------|----------|--------|-------|
| MNIST  | FAST      | 6s                | 96.62%   | 1.0021 | 113.86|
| MNIST  | ECO       | 21s               | 97.65%   | 1.0015 | 117.63|
| MNIST  | STRONG    | 211s              | 96.85%   | 1.0020 | 105.09|
| SIFT   | FAST      | ~10 λεπ           | 86.00%   | 1.0251 | 36.63 |
| SIFT   | ECO       | ~20 λεπ           | 95.00%   | 1.0011 | 36.24 |
| SIFT   | STRONG    | ~11 ώρες          | 93.00%   | 1.0065 | 35.95 |

**Συμπέρασμα:**
- **MNIST**: ECO mode βέλτιστη (καλύτερη ισορροπία recall-ταχύτητας)
- **SIFT**: ECO mode ανώτερη (+9% recall vs FAST, ίδια ταχύτητα με STRONG)
- **STRONG**: Περίσσεια για μικρότερα σύνολα, ελάχιστα κέρδη στο SIFT
...
**Για το Neural LSH:**
- Κατάλληλο για σύνολα δεδομένων 50k-5M σημείων
- Ιδανικό όταν απαιτείται υψηλό recall (>95%)
- Επιτυγχάνει καλή απόδοση με φυσικές δομές δεδομένων που υποστηρίζουν διαμέριση γράφου
- Όταν ο χρόνος κατασκευής δεν είναι κρίσιμος παράγοντας
- Αποδεκτή πρακτική απόδοση ερωτημάτων (10-100 QPS)

**Για το IVFFlat:**
- Όταν απαιτείται μέγιστο recall (>99%)
- Για εφαρμογές με υψηλές απαιτήσεις απόδοσης ερωτημάτων (>150 QPS)
- Εάν υπάρχει δυνατότητα ρύθμισης των παραμέτρων nlist/nprobe

**Για το LSH:**
- Όταν ο χρόνος κατασκευής είναι κρίσιμος παράγοντας
- Για σύνολα δεδομένων >10M σημείων
- Εάν ένα μέσο recall (80-90%) είναι αποδεκτό
